...
export default function EventTicketsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch event
        const eventResponse = await eventService.getEventById(eventId, token);
        if (eventResponse.success) {
          setEvent(eventResponse.data);
        }

        // Fetch tickets
        const ticketsResponse = await ticketService.getEventTickets(eventId, token);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (maxPerUser <= 0) throw new Error('Max per user must be greater than 0');
      ...
      if (editingTicket) {
        // Update ticket
        const response = await ticketService.updateTicket(editingTicket._id, ticketData, token);
        setTickets(tickets.map(t => t._id === editingTicket._id ? response.data : t));
        setSuccess('Ticket updated successfully!');
      } else {
        // Create new ticket
        const response = await ticketService.createTicket(eventId, ticketData, token);
    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 pt-28 pb-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-2">Manage Tickets</h1>
          ...
          <div className="lg:col-span-2">
            {tickets.length === 0 && !showForm ? (
              <div className="bg-gray-900/50 border border-gold/20 rounded-xl p-12 text-center">
                <p className="text-gray-400 mb-4">No tickets created yet</p>
                <button
              ...
              <>
                {tickets.length > 0 && (
                  <div className="space-y-4">
                    {tickets.map((ticket) => (
                      <div key={ticket._id} className="bg-gray-900/50 border border-gold/20 rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-3 text-sm">
                          <div>
                            ...
                            <p className="text-gold font-semibold">₦{ticket.price.toLocaleString()}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Available:</span>
                            <p className="text-white font-semibold">{ticket.quantity - ticket.sold} / {ticket.quantity}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Sold:</span>
                            <p className="text-green-400 font-semibold">{ticket.sold}</p>
                          </div>
                          <div>
                            <span className="text-gray-400">Max per user:</span>
                            <p className="text-white font-semibold">{ticket.maxPerUser}</p>
                          </div>
                        </div>
          ...
          {showForm && (
            <div className="bg-gray-900/50 border border-gold/20 rounded-xl p-6 h-fit sticky top-28">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold">{editingTicket ? 'Edit Ticket' : 'Create Ticket'}</h2>
                <button
                  onClick={() => {
                    setShowForm(false);
                    resetForm();
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Max Per User */}
                <div>
                  <label className="block text-sm font-medium mb-2">Max Per User</label>
                  <input
                    type="number"
                    name="maxPerUser"
