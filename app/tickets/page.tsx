...
export default function TicketsPage() {
    ...
    const fetchTickets = async () => {
      try {
        ...
        if (tx_ref || lastPayment?.tx_ref) {
          ...
          try {
            const response = await fetch(`https://skills-k6pv.onrender.com/api/tickets/${ticketRef}`);
            ...
            if (response.ok) {
              ...
              const formattedTicket: TicketData = {
                ...
                customerEmail: ticketData.customerEmail || lastPayment?.email || 'guest@example.com',
                ticketName: ticketData.ticketName || lastPayment?.ticketName || 'Event Ticket',
                eventDate: ticketData.eventDate || 'March 27, 2026',
                eventName: ticketData.eventName || 'LOFTE-3 Dinner Night',
                eventLocation: ticketData.eventLocation || 'Eko Hotels & Suites, Lagos',
        ...
        try {
          const response = await fetch(`https://skills-k6pv.onrender.com/api/tickets/user/guest`);
  ...
  const downloadTicket = (ticket: TicketData) => {
    alert('Download feature coming soon! Your ticket QR code will be available for download.');
  };

  const shareTicket = (ticket: TicketData) => {
    alert('Share feature coming soon! You will be able to share your ticket via email.');
  };

  const viewTicketDetails = (ticket: TicketData) => {
    setSelectedTicket(selectedTicket?.ticketId === ticket.ticketId ? null : ticket);
  };
        ...
        <div className="container mx-auto px-4 pt-32 pb-20">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center max-w-md">
              ...
              <h2 className="text-2xl font-bold text-red-500 mb-4">Error Loading Tickets</h2>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gold">
                  ₦{tickets.reduce((acc, t) => acc + t.amount, 0).toLocaleString()}
                </p>
              </div>
              <div className="p-3 bg-gold/20 rounded-lg">
        ...
        {filteredTickets.length > 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-white">{ticket.ticketName}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-white">{ticket.customerEmail}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-white">
                          {ticket.currency === 'NGN' ? '₦' : '$'}{ticket.amount.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      >
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            ...
                            <span className="text-white">{ticket.eventLocation}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          {ticket.tx_ref && (
                            <div className="flex justify-between">
                              ...
                              <span className="text-gold font-mono text-xs">{ticket.tx_ref}</span>
                            </div>
                          )}
                        </div>
                      </motion.div>
          ...
          >
            ...
            <p className="text-gray-400 mb-8 max-w-md mx-auto">
              You haven't purchased any tickets yet. Browse our upcoming events and get your tickets now!
            </p>
            <Link
              href="/#events"
