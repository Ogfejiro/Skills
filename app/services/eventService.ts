...
class EventService {
	...
	async getHostEvents(
		...
		page: number = 1,
		limit: number = 10,
	): Promise<GetHostEventsResponse> {
		try {
			console.log('📖 Fetching host events:', { page, limit })

			const response = await fetch(
				`${this.baseUrl}/api/events/host?page=${page}&limit=${limit}`,
				{
					method: 'GET',
					headers: {
						'Content-Type': 'application/json',
	...
	async getPublicEvents(
		...
		try {
			...
			const response = await fetch(
				`${this.baseUrl}/api/event-public?page=${page}&limit=${limit}`,
				{
					method: 'GET',
					headers: {
						'Content-Type': 'application/json',
			...
			if (!response.ok) {
				...
				throw new Error(
					error.message || 'Failed to fetch public events',
				)
			}
